				...
				result.map.objectgroup.forEach(function(event_layer){
					...
					if(event_layer['$']['name'] == 'events'){
						...
						event_layer['object'].forEach(function(object){
							...
							//	Create event cell
							if(events[object_x] == undefined){ events[object_x] = [] }
							if(events[object_x][object_y] == undefined){ events[object_x][object_y] = [] }
...
function detect_event(player, zone, x, y){
	...
	if(zones[zone].events[x] == undefined){ return false; }
	if(zones[zone].events[x][y] == undefined){ return false; }
